...
## Please install "fpc" package: install.packages("fpc") for decision trees                                                                                              #
...
## LOAD THE DATA
zip.data <- read.csv("C:/Users/muckam/Desktop/DataScienceBootcamp/Datasets/Zip/zip.train.csv", header= F)
...
## scale the data
zip.train <- as.data.frame(scale(zip.data.subset[,-1])) #scale data to normalized range, helps with fast computation. scale is like normalize, but subtle distinction
## Run the K-means clustering model
zip.km.model <- kmeans(zip.train, 10, iter.max = 100, nstart = 25) #k=10, it will try 25 times to start it, then give up
